...
from workshop_pro.engines_subsystem.engines import Engine, GasEngine, ElectricEngine, HybridEngine
...
class TestEngine:
    ...
    def test_liskov_hybrid_engine(self):
        """This is a test case to verify the creation of a hybrid engine"""
        new_hybrid_engine = HybridEngine(
            torque=self.data_test["torque"],
            maximum_speed=self.data_test["maximum_speed"],
            dimenssions=self.data_test["dimenssions"],
            power=self.data_test["power"],
            stability=self.data_test["stability"],
            weight=self.data_test["weight"],
            electric_power=self.data_test["electric_power"]
        )

        assert isinstance(new_hybrid_engine, HybridEngine)
        assert isinstance(new_hybrid_engine, Engine)
